...
parser.add_argument('--model_path', default=
    '../model/version-RFB/RFB-320.mnn', type=str, help='model path')
...
def predict(width, height, confidences, boxes, prob_threshold,
    ...
    for class_index in range(1, confidences.shape[1]):
        ...
        box_probs = box_utils.hard_nms(box_probs, iou_threshold=
            iou_threshold, top_k=top_k)
...
def inference():
    ...
    for file_path in listdir:
        ...
        tmp_input = MNN.Tensor((1, 3, input_size[1], input_size[0]), MNN.
            Halide_Type_Float, image, MNN.Tensor_DimensionType_Caffe)
        ...
        boxes, labels, probs = predict(image_ori.shape[1], image_ori.shape[
            0], scores, boxes, args.threshold)
        for i in range(boxes.shape[0]):
            box = boxes[i, :]
            cv2.rectangle(image_ori, (box[0], box[1]), (box[2], box[3]), (0,
                255, 0), 2)
