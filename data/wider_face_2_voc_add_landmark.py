...
def convertimgset(img_set='train'):
    ...
    with open(gtfilepath, 'r') as gtfile:
        while True:
            ...
            if line == '':
                if len(bboxes) != 0:
                    method_name(bboxes, filename, saveimg, vocannotationdir,
                        lms, img_set)
            ...
            if line.startswith('#'):
                if index != 0 and convert2vocformat:
                    if len(bboxes) != 0:
                        method_name(bboxes, filename, saveimg,
                            vocannotationdir, lms, img_set)
                        cv2.imwrite(imagesdir + '/' + filename, saveimg)
                        imgfilepath = filename[:-4]
                        f_set.write(imgfilepath + '\n')
                    else:
                ...
                if width >= minsize2select and height >= minsize2select:
                    ...
                    if img_set == 'train':
                        if line[4] == -1:
                            ...
                            lm.append(line[19])
                            lms.append(lm)
                    cv2.rectangle(showimg, (int(x), int(y)), (int(x2), int(
                        y2)), (0, 255, 0))
                else:
                ...
                if convet2yoloformat:
                    ...
                    for i in range(len(bboxes)):
                        ...
                        txtline = '0 ' + str(xcenter) + ' ' + str(ycenter
                            ) + ' ' + str(wr) + ' ' + str(hr) + '\n'
                        ftxt.write(txtline)
                    ftxt.close()
    f_set.close()
...
def generatetxt(img_set='train'):
    gtfilepath = (rootdir + '/wider_face_split/wider_face_' + img_set +
        '_bbx_gt.txt')
...
if __name__ == '__main__':
    convertdataset()
    shutil.move(rootdir + '/ImageSets/Main/' + 'train.txt', rootdir +
        '/ImageSets/Main/' + 'trainval.txt')
    shutil.move(rootdir + '/ImageSets/Main/' + 'val.txt', rootdir +
