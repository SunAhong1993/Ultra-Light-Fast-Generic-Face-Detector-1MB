...
parser.add_argument('--input_size', default=480, type=int, help=
...
parser.add_argument('--threshold', default=0.7, type=float, help=
    'score threshold')
parser.add_argument('--candidate_size', default=1000, type=int, help=
    'nms candidate size')
...
parser.add_argument('--video_path', default=
    '/home/linzai/Videos/video/16_1.MP4', type=str, help='path of video')
...
while True:
    ...
    boxes, labels, probs = predictor.predict(image, candidate_size / 2,
        threshold)
    interval = timer.end()
    print('Time: {:.6f}s, Detect Objects: {:d}.'.format(interval, labels.
        size(0)))
    for i in range(boxes.size(0)):
        box = boxes[i, :]
        label = f' {probs[i]:.2f}'
        cv2.rectangle(orig_image, (box[0], box[1]), (box[2], box[3]), (0, 
            255, 0), 4)
