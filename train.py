...
from x2paddle.torch2paddle import ConcatDataset
from vision.datasets.voc_dataset import VOCDataset
from vision.nn.multibox_loss import MultiboxLoss
from vision.ssd.config.fd_config import define_img_size
from vision.utils.misc import str2bool
from vision.utils.misc import Timer
from vision.utils.misc import freeze_net_layers
from vision.utils.misc import store_labels
parser = argparse.ArgumentParser(description='train With Pytorch')
parser.add_argument('--dataset_type', default='voc', type=str, help=
    'Specify dataset type. Currently support voc.')
parser.add_argument('--datasets', nargs='+', help='Dataset directory path')
parser.add_argument('--validation_dataset', help='Dataset directory path')
parser.add_argument('--balance_data', action='store_true', help=
    'Balance training data by down-sampling more frequent labels.')
parser.add_argument('--net', default='RFB', help=
    'The network architecture ,optional(RFB , slim)')
parser.add_argument('--freeze_base_net', action='store_true', help=
    'Freeze base net layers.')
parser.add_argument('--freeze_net', action='store_true', help=
    'Freeze all the layers except the prediction head.')
...
parser.add_argument('--base_net_lr', default=None, type=float, help=
    'initial learning rate for base net.')
parser.add_argument('--extra_layers_lr', default=None, type=float, help=
...
parser.add_argument('--scheduler', default='multi-step', type=str, help=
    'Scheduler for SGD. It can one of multi-step and cosine')
parser.add_argument('--milestones', default='80,100', type=str, help=
    'milestones for MultiStepLR')
parser.add_argument('--t_max', default=120, type=float, help=
...
parser.add_argument('--validation_epochs', default=5, type=int, help=
    'the number epochs')
parser.add_argument('--debug_steps', default=100, type=int, help=
    'Set the debug log output frequency.')
parser.add_argument('--use_cuda', default=True, type=str2bool, help=
    'Use CUDA to train model')
parser.add_argument('--checkpoint_folder', default='models/', help=
    'Directory for saving checkpoint models')
parser.add_argument('--log_dir', default=
    './models/Ultra-Light(1MB)_&_Fast_Face_Detector/logs', help='lod dir')
parser.add_argument('--cuda_index', default='0', type=str, help=
    'Choose cuda index.If you have 4 GPUs, you can set it like 0,1,2,3')
parser.add_argument('--power', default=2, type=int, help='poly lr pow')
parser.add_argument('--overlap_threshold', default=0.35, type=float, help=
    'overlap_threshold')
parser.add_argument('--optimizer_type', default='SGD', type=str, help=
    'optimizer_type')
parser.add_argument('--input_size', default=320, type=int, help=
...
DEVICE = 'cuda:0' if paddle.is_compiled_with_cuda(
    ) and args.use_cuda else 'cpu'
DEVICE = DEVICE.replace('cuda', 'gpu')
DEVICE = paddle.set_device(DEVICE)
if args.use_cuda and paddle.is_compiled_with_cuda():
...
if __name__ == '__main__':
    ...
    if args.net == 'slim':
        ...
        sys.exit(1)
    train_transform = TrainAugmentation(config.image_size, config.
        image_mean, config.image_std)
    target_transform = MatchPrior(config.priors, config.center_variance,
        config.size_variance, args.overlap_threshold)
    test_transform = TestTransform(config.image_size, config.
        image_mean_test, config.image_std)
    ...
    train_loader = DataLoader(train_dataset, args.batch_size, num_workers=
        args.num_workers, shuffle=True)
    logging.info('Prepare Validation datasets.')
    if args.dataset_type == 'voc':
        val_dataset = VOCDataset(args.validation_dataset, transform=
            test_transform, target_transform=target_transform, is_test=True)
    logging.info('validation dataset size: {}'.format(len(val_dataset)))
    val_loader = DataLoader(val_dataset, args.batch_size, num_workers=args.
        num_workers, shuffle=False)
    logging.info('Build network.')
    net = create_net(num_classes)
    if torch2paddle.device_count() >= 1:
    ...
    if args.freeze_base_net:
        ...
        params = itertools.chain(net.source_layer_add_ons.parameters(), net
            .extras.parameters(), net.regression_headers.parameters(), net.
            classification_headers.parameters())
        params = [{'params': itertools.chain(net.source_layer_add_ons.
            parameters(), net.extras.parameters()), 'lr': extra_layers_lr},
        ...
        params = [{'params': net.module.base_net.parameters(), 'lr':
            base_net_lr}, {'params': itertools.chain(net.module.
            source_layer_add_ons.parameters(), net.module.extras.parameters
            ()), 'lr': extra_layers_lr}, {'params': itertools.chain(net.
            module.regression_headers.parameters(), net.module.
            classification_headers.parameters())}]
    ...
    if args.optimizer_type == 'SGD':
        optimizer = torch2paddle.Momentum(params, lr=args.lr, momentum=args
            .momentum, weight_decay=args.weight_decay)
    elif args.optimizer_type == 'Adam':
        optimizer = torch2paddle.Adam(params, lr=args.lr)
    ...
    if args.optimizer_type != 'Adam':
        if args.scheduler == 'multi-step':
            ...
            scheduler = MultiStepDecay(milestones=milestones, gamma=0.1,
                last_epoch=last_epoch, learning_rate=0.01)
            optimizer._learning_rate = scheduler
        elif args.scheduler == 'cosine':
            logging.info('Uses CosineAnnealingLR scheduler.')
            scheduler = CosineAnnealingDecay(last_epoch=last_epoch, T_max=
                args.t_max, learning_rate=0.01)
            optimizer._learning_rate = scheduler
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            model_path = os.path.join(args.checkpoint_folder,
                f'{args.net}-Epoch-{epoch}-Loss-{val_loss}.pdiparams')
