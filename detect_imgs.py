...
parser.add_argument('--net_type', default='RFB', type=str, help=
    'The network architecture ,optional: RFB (higher precision) or slim (faster)'
    )
parser.add_argument('--input_size', default=640, type=int, help=
...
parser.add_argument('--threshold', default=0.6, type=float, help=
    'score threshold')
parser.add_argument('--candidate_size', default=1500, type=int, help=
    'nms candidate size')
parser.add_argument('--path', default='imgs', type=str, help='imgs dir')
parser.add_argument('--test_device', default='cuda:0', type=str, help=
    'cuda:0 or cpu')
...
if args.net_type == 'slim':
    ...
    predictor = create_mb_tiny_fd_predictor(net, candidate_size=args.
    ...
    net = create_Mb_Tiny_RFB_fd(len(class_names), is_test=True, device=
        test_device)
    predictor = create_Mb_Tiny_RFB_fd_predictor(net, candidate_size=args.
        candidate_size, device=test_device)
else:
...
for file_path in listdir:
    ...
    for i in range(boxes.size(0)):
        box = boxes[i, :]
        cv2.rectangle(orig_image, (box[0], box[1]), (box[2], box[3]), (0, 0,
            255), 2)
        label = f'{probs[i]:.2f}'
    cv2.putText(orig_image, str(boxes.size(0)), (30, 30), cv2.
