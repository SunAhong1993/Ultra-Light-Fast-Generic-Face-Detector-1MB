...
def post_processing(reg_list, cls_list, num_classes, image_size,
    feature_map_wh_list, min_boxes, center_variance, size_variance,
    conf_threshold=0.6, nms_max_output_size=100, nms_iou_threshold=0.3,
    top_k=100):
    reg_list = [tf.keras.layers.Reshape([-1, 4])(reg) for reg in reg_list]
    cls_list = [tf.keras.layers.Reshape([-1, num_classes])(cls) for cls in
        cls_list]
    ...
    mask = tf.image.non_max_suppression(boxes=result[..., -4:], scores=
        result[..., 1], max_output_size=nms_max_output_size, iou_threshold=
        nms_iou_threshold, name='non_maximum_suppresion')
...
def decode_regression(reg, image_size, feature_map_w_h_list, min_boxes,
    ...
    for feature_map_w_h, min_box in zip(feature_map_w_h_list, min_boxes):
        xy_grid = np.meshgrid(range(feature_map_w_h[0]), range(
            feature_map_w_h[1]))
        ...
        wh_grid = np.tile(np.transpose(wh_grid), [np.product(
            feature_map_w_h), 1])
        prior = np.concatenate((xy_grid, wh_grid), axis=-1)
        priors.append(prior)
    priors = np.concatenate(priors, axis=0)
    print(f'priors nums:{priors.shape[0]}')
    priors = tf.constant(priors, dtype=tf.float32, shape=priors.shape, name
        ='priors')
    center_xy = reg[..., :2] * center_variance * priors[..., 2:] + priors[...,
        :2]
...
def load_weight(model, torch_path, mapping_table_path):
    torch_weights = paddle.load(torch_path)
    with open(mapping_table_path, 'r') as f:
        mapping_table = json.load(f)
        mapping_table = {layer['name']: layer['weight'] for layer in
            mapping_table}
