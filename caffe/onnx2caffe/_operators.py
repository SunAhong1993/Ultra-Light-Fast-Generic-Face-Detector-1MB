...
def make_input(input):
    ...
    shape = input[2]
    shape = list(shape)
    input_layer = myf('Input', name, [], output, input_param=dict(shape=
        dict(dim=shape)))
    return input_layer
...
def _convert_conv(node, graph, err):
    ...
    layer = myf('Convolution', node_name, [input_name], [output_name],
        kernel_h=kernel_shape[0], kernel_w=kernel_shape[1], stride_h=
        strides[0], stride_w=strides[1], group=groups, pad_h=pads[0], pad_w
        =pads[1], num_output=W.shape[0], dilation=dilations[0], bias_term=
        bias_flag)
...
def _convert_BatchNorm(node, graph, err):
    ...
    if input_name == output_name:
        ...
        inplace = False
    bn_layer = myf('BatchNorm', node_name + '_bn', [input_name], [
        output_name], eps=epsilon, use_global_stats=True, in_place=inplace)
    scale_layer = myf('Scale', node_name, [output_name], [output_name],
        in_place=True, bias_term=True)
    graph.channel_dims[output_name] = graph.channel_dims[input_name]
        ...
        if node.attrs['broadcast'] == 1:
            ...
            if input_node_number != 2:
                return err.unsupported_op_configuration(node,
                    'Broadcast Add must has 2 input, not {}'.format(
                    input_node_number))
            ...
            layer = myf('Bias', node_name, [input_name_list[0], output_name +
                '_flat'], [output_name], axis=axis)
            graph.channel_dims[output_name] = graph.channel_dims[
                input_name_list[0]]
            return flat_layer, layer
    layer = myf('Eltwise', node_name, input_name_list, [output_name],
...
def _convert_Mul(node, graph, err):
    ...
    if 'broadcast' in node.attrs:
        if node.attrs['broadcast'] == 1:
            ...
            if input_node_number != 2:
                return err.unsupported_op_configuration(node,
                    'Broadcast Mul must has 2 input, not {}'.format(
                    input_node_number))
            axis = node.attrs['axis']
            flat_layer = myf('Flatten', node_name + '_flat', [
                input_name_list[1]], [output_name + '_flat'])
            layer = myf('Scale', node_name, [input_name_list[0], 
                output_name + '_flat'], [output_name], bias_term=False,
                axis=axis)
            graph.channel_dims[output_name] = graph.channel_dims[
                input_name_list[0]]
            return flat_layer, layer
...
def _convert_Reshape(node, graph, err):
    ...
    if input_name == output_name:
        ...
        inplace = False
    if len(shape) == 2:
        layer = myf('Flatten', node_name, [input_name], [output_name],
            in_place=inplace)
        graph.channel_dims[output_name] = shape[1]
        return layer
    elif len(shape) == 4 or len(shape) == 3:
        graph.channel_dims[output_name] = shape[1]
        layer = myf('Reshape', node_name, [input_name], [output_name],
...
def _convert_Flatten(node, graph, err):
    ...
    if input_name == output_name:
        ...
        inplace = False
    layer = myf('Flatten', node_name, [input_name], [output_name], in_place
        =inplace)
    return layer
...
def _convert_Permute(node, graph, err):
    ...
    if len(shape) == 4:
        layer = myf('Permute', node_name, [input_name], [output_name],
            permute_param=dict(order=list(shape)))
        return layer
    else:
        return err.unsupported_op_configuration(node,
            'Reshape dimention number shall be 2 or 4')
...
def _convert_pool(node, graph, err):
    ...
    layer = myf('Pooling', node_name, [input_name], [output_name],
        pooling_param=dict(pool=pool_type, kernel_h=kernel_shape[0],
        kernel_w=kernel_shape[1], stride_h=strides[0], stride_w=strides[1],
...
def _convert_dropout(node, graph, err):
    ...
    layer = myf('Dropout', node_name, [input_name], [output_name],
        dropout_ratio=ratio)
    graph.channel_dims[output_name] = graph.channel_dims[input_name]
    return layer
...
def _convert_gemm(node, graph, err):
    ...
    if b is not None:
        ...
        if W.shape[0] != b.shape[0]:
            return err.unsupported_op_configuration(node,
                'Gemm is supported only for inner_product layer')
    layer = myf('InnerProduct', node_name, [input_name], [output_name],
        num_output=W.shape[0], bias_term=bias_flag)
    graph.channel_dims[output_name] = W.shape[0]
    return layer
...
def _convert_upsample(node, graph, err):
    ...
    if mode == 'bilinear':
        layer = myf('Deconvolution', node_name, [input_name], [output_name],
            convolution_param=dict(num_output=channels, kernel_size=2 *
            factor - factor % 2, stride=factor, pad=pad, group=channels,
            bias_term=False, weight_filler=dict(type='bilinear_upsampling')))
    else:
        layer = myf('Deconvolution', node_name, [input_name], [output_name],
            convolution_param=dict(num_output=channels, kernel_size=factor,
            stride=factor, group=channels, bias_term=False))
    graph.channel_dims[output_name] = graph.channel_dims[input_name]
    return layer
...
def _convert_conv_transpose(node, graph, err):
    ...
    strides = node.attrs['strides']
    layer = myf('Deconvolution', node_name, [input_name], [output_name],
        convolution_param=dict(num_output=W.shape[1], kernel_h=kernel_shape
        [0], kernel_w=kernel_shape[1], stride_h=strides[0], stride_w=
        strides[1], group=groups, pad_h=pads[0], pad_w=pads[1], bias_term=
        bias_flag))
    graph.channel_dims[output_name] = W.shape[1]
    return layer


_ONNX_NODE_REGISTRY = {'Conv': _convert_conv, 'Relu': _convert_relu,
    'BatchNormalization': _convert_BatchNorm, 'Add': _convert_Add, 'Mul':
    _convert_Mul, 'Reshape': _convert_Reshape, 'MaxPool': _convert_pool,
    'AveragePool': _convert_pool, 'Dropout': _convert_dropout, 'Gemm':
    _convert_gemm, 'Upsample': _convert_upsample, 'Concat': _convert_concat,
    'ConvTranspose': _convert_conv_transpose, 'Sigmoid': _convert_sigmoid,
    'Flatten': _convert_Flatten, 'Transpose': _convert_Permute, 'Softmax':
    _convert_Softmax}
