...
class NodesFuser(object):
    ...
    def __call__(self, graph):
        ...
        return Graph(transformed_nodes, graph.inputs, graph.outputs, graph.
            shape_dict)
...
class ConvAddFuser(NodesFuser):
    ...
    def is_eligible(self, graph, nodes):
        ...
        if parent.inputs[1] not in parent.input_tensors:
            return False
        if len(parent.inputs) > 2 and parent.inputs[2
            ] not in parent.input_tensors:
            return False
    ...
    def __call__(self, graph):
        ...
        for node in nodes:
            if node.op_type != 'Reshape':
                continue
            if not (len(node.input_tensors) == 2 or len(node.input_tensors) ==
                1):
                continue
...
class PixelShuffleFuser(NodesFuser):
    ...
    def merge(self, graph, nodes):
        ...
        height = shape[4]
        width = shape[5]
        reshape_1.input_tensors[reshape_1.inputs[1]] = np.asarray([channels,
        ...
        transpose_1.outputs = [self.get_unique_edge_name(graph,
            transpose_output_name)]
        shape_name_second_reshape = self.get_unique_edge_name(graph,
            reshape_output_name)
        output_name_second_reshape = self.get_unique_edge_name(graph,
            reshape_output_name)
        reshape_2 = Node(reshape_output_name, 'Reshape', {}, [transpose_1.
            outputs[0], shape_name_second_reshape], [
            output_name_second_reshape])
        reshape_2.input_tensors[shape_name_second_reshape] = np.asarray([
            channels * height, scale, scale, width])
        transpose_1.add_child(reshape_2)
        transpose_2 = Node(transpose_output_name, 'Transpose', {'perm': [0,
            1, 3, 2]}, reshape_2.outputs, [self.get_unique_edge_name(graph,
            transpose_output_name)])
    ...
    def __call__(self, graph):
        ...
        for node in graph.nodes:
            if str(node.op_type) == 'LSTM':
                input_h = node.inputs[5] if len(node.inputs
                    ) > 5 else node.inputs[0] + '_h_input'
                input_c = node.inputs[6] if len(node.inputs
                    ) > 6 else node.inputs[0] + '_c_input'
                output_h = node.outputs[1] if len(node.outputs
                    ) > 1 else node.outputs[0] + '_h_output'
                output_c = node.outputs[2] if len(node.outputs
                    ) > 2 else node.outputs[0] + '_c_output'
                h = node.attrs['hidden_size']
                for input_ in [str(input_h), str(input_c)]:
                    if input_ not in input_names:
                        graph.inputs.append(tuple((input_, TensorProto.
                            FLOAT, (h,))))
                ...
                for output_ in [str(output_h), str(output_c)]:
                    if output_ not in output_names:
                        graph.outputs.append(tuple((output_, TensorProto.
                            FLOAT, (h,))))
...
class ImageScalerRemover(object):
    ...
    def __call__(self, graph):
        ...
        nodes_to_be_removed = []
        for node in graph.nodes:
            if node.op_type != 'ImageScaler' or len(node.parents
                ) != 0 or node.inputs[0] not in input_names:
                continue
            is_eligible = True
            for child in node.children:
                if not (len(child.parents) == 1 and child.inputs[0] == node
                    .outputs[0]):
                    is_eligible = False
                    break
